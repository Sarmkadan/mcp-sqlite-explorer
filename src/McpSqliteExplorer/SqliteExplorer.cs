...
public sealed partial class SqliteExplorer : IDisposable, ISqliteCatalog
{
    ...
    private static bool IsPathWithinSafeBoundaries(string path)
    {
        try
        {
            ...
            // Also check if the path resolves to a location that's clearly outside reasonable bounds
            // For example, if it starts with a drive letter and goes to a different drive
            // or if it contains multiple absolute path roots
    ...
    private static string ResolveSymlinks(string path)
    {
        try
        {
            ...
            // This works on Windows and Unix-like systems
            var fileInfo = new FileInfo(path);
            if (fileInfo.Exists)
            {
                // Use GetFinalPathName to resolve symlinks and junctions
                // This is a Windows-specific API, but FileInfo handles it cross-platform
    ...
    public SqliteExplorer(string databasePath)
    {
        ...
        var baseCatalog = new SqliteCatalog(_validatedDatabasePath);
